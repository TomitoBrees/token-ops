...
import { companyInvitations, companyMembers, MEMBER_ROLES, profiles } from "@/drizzle/schema";
...
export const invitationRouter = createTRPCRouter({
    ...
    .mutation(async ({ctx, input}) => {
        ...
        const [invitation] = await ctx.db.transaction(async (tx) => {
            ...
            return await tx.insert(companyInvitations).values({email: input.email, role: input.role, company_id: adminMembership.companyId}).returning()
        })
    ...
    .input(z.object({ invitationId: z.uuid(), displayName: z.string().min(1) }))
    .mutation(async ({ ctx, input }) => {
        ...
        if (!invitation.company_id) {
            throw new TRPCError({ code: 'BAD_REQUEST', message: 'Invitation has no company' })
        }
        ...
        const userEmail = profile.email ?? (ctx.user as { email?: string }).email
        if (!userEmail || userEmail.toLowerCase() !== invitation.email.toLowerCase()) {
            throw new TRPCError({ code: 'FORBIDDEN', message: 'Invitation email does not match your account' })
        }
    ...
    .query(async ({ctx, input}) => {
        return await ctx.db.query.companyInvitations.findFirst({
